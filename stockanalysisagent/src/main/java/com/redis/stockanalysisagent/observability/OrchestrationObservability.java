...
public final class OrchestrationObservability {
    ...
    public static void enrichWithAgentExecution(Observation observation, AgentExecution execution) {
        enrichWithAgentExecution(observation, execution, "COMPLETED");
    }

    public static void enrichWithAgentExecution(Observation observation, AgentExecution execution, String agentStatus) {
        if (execution == null) {
            return;
        }
        if (agentStatus != null && !agentStatus.isBlank()) {
            observation.lowCardinalityKeyValue(KEY_AGENT_STATUS, agentStatus);
        }
